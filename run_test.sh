...
docker-compose up -d --build --force-recreate agg-metrics-cmd
echo "Waiting for agg-metrics-cmd to initialize"
sleep 5

docker-compose up -d --build --force-recreate agg-metrics-query
sleep 5

docker ps -a

docker-compose up --exit-code-from  agg-metrics-query-test
rc=$?
if [[ $rc != 0 ]]
  docker ps -a
  then exit $rc
