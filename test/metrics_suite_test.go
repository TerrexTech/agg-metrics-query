...
import (
	...
	"github.com/TerrexTech/agg-metrics-query/metrics"
...
func TestUser(t *testing.T) {
	...
	RunSpecs(t, "MetricsAggregate Suite")
}

var _ = Describe("MetricsAggregate", func() {
	var (
		kafkaBrokers          []string
		eventsTopic           string
	...
	BeforeSuite(func() {
		...
		marshalMetrics, err := json.Marshal(mockMetrics)
		Expect(err).ToNot(HaveOccurred())
	...
	Describe("Metric Operations", func() {
		It("should query record", func(done Done) {
			...
			queryArgs := map[string]interface{}{
				"metricID": mockMetrics.MetricID, //Should I do itemID? -- check
			}
			...
			c, err := kafka.NewConsumer(&kafka.ConsumerConfig{
				KafkaBrokers: kafkaBrokers,
				GroupName:    "aggmetrics.test.group.1",
			...
			msgCallback := func(msg *sarama.ConsumerMessage) bool {
				...
				if kr.UUID == mockEvent.TimeUUID {
					...
					for _, r := range result {
						if r.MetricID == mockMetrics.MetricID {
							mockMetrics.ID = r.ID
							Expect(r).To(Equal(*mockMetrics))
							return true
						}
					}
				}
